...
export default function LocationPage({ formData, setFormData }: LocationPageProps) {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Location Preferences</h2>
      ...
      <div>
        ...
        <select
          ...
          required
        >
          <option value="" disabled className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">Select a region</option>
          {Object.keys(HOUSING_REGIONS).map(region => (
            <option key={region} value={region} className="bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{region}</option>
          ))}
        </select>
      </div>
                ...
                <div className="mt-4">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      id="customCity"
                      className="flex-1 rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-2 text-gray-900 dark:text-gray-100 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                ...
                <div className="mt-4">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {formData.housingCities.filter(city => !availableCities.includes(city)).map(city => (
                      <div
                        key={city}
                        className="inline-flex items-center rounded-full bg-blue-100 dark:bg-blue-900/50 px-3 py-1 text-sm text-blue-800 dark:text-blue-200"
                      >
                        {city}
                        <button
                          type="button"
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/30 p-3 rounded-lg">
        <p className="text-sm text-yellow-800 dark:text-yellow-300">
          We'll use this information to match you with others looking in the same areas.
        </p>
      </div>
    </div>
  );
